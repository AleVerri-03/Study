...
Poisson2D::assemble()
{
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  // Stiffness matrix: alpha * (grad u, grad v)
                  cell_matrix(i, j) += alpha * fe_values.shape_grad(i, q) *
                                       fe_values.shape_grad(j, q) *
                                       fe_values.JxW(q);

                  // Reaction term: reaction_coefficient * (u, v)
                  cell_matrix(i, j) += reaction_coefficient *
              ...
              // Forcing term: (f, v)
              cell_rhs(i) +=
                forcing_term * fe_values.shape_value(i, q) * fe_values.JxW(q);
            }
        }
  ...
  // Boundary conditions: u = 0 on external boundaries.
  {
    std::map<types::global_dof_index, double>           boundary_values;
    std::map<types::boundary_id, const Function<dim> *> boundary_functions;

    Functions::ZeroFunction<dim> zero_function;
    boundary_functions[subdomain_id == 0 ? 0 : 1] = &zero_function;
