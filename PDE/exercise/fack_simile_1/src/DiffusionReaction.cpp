...
DiffusionReaction::assemble()
{
  ...
  // Dirichlet boundary conditions.
  {
    ...
    {
    public:
      GFunction(const std::function<double(const Point<dim> &)> &g_)
        : g(g_)
      {}
    ...
    private:
      const std::function<double(const Point<dim> &)> &g;
    };

    GFunction g_function(g);

    std::map<types::boundary_id, const Function<dim> *> boundary_functions;
    boundary_functions[0] = &bc_function;  // x = 0, Dirichlet omogenea
    boundary_functions[2] = &bc_function;  // y = 0, Dirichlet omogenea
    boundary_functions[3] = &g_function;   // y = 1, Dirichlet = g
