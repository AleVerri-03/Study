  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      // --- QUESTO È IL CICLO CHE PROBABILMENTE MANCAVA O ERA SCRITTO MALE ---
      for (unsigned int q = 0; q < n_q; ++q)
        {
          // Coordinata x del punto di quadratura corrente
          const double x_q = fe_values.quadrature_point(q)[0];
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  // Termine Diffusivo: u' * v'
                  double diffusion = fe_values.shape_grad(j, q)[0] * 
                                     fe_values.shape_grad(i, q)[0];
                                     
                  // Termine Convettivo: -x * u' * v
                  double convection = - x_q * 
                                      fe_values.shape_grad(j, q)[0] * 
                                      fe_values.shape_value(i, q);
                                      
                  // Termine Reattivo: + u * v
                  double reaction = fe_values.shape_value(j, q) * 
                                    fe_values.shape_value(i, q);

                  cell_matrix(i, j) += (diffusion + convection + reaction) * 
                                       fe_values.JxW(q);
                }
