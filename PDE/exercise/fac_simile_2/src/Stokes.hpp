...
public:
  ...
  public:
    ...
    {
      // g(x1, x2) = (27/4 * x2 * (1 - x2)^2, 0)^T on Gamma_0 (Inlet)
      values[0] = 27.0 / 4.0 * p[1] * std::pow(1.0 - p[1], 2);
      values[1] = 0.0;
      values[2] = 0.0; // Pressure component (not used for BC but required by size)
    }
    ...
    {
      if (component == 0)
        return 27.0 / 4.0 * p[1] * std::pow(1.0 - p[1], 2);
      else
        return 0.0;
    }
