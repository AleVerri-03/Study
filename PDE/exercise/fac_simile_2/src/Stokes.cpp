...
Stokes::setup()
{
  // Create the mesh.
  {
    ...
    // [MODIFICA] Generate a structured mesh instead of reading from file
    // to avoid "Element Identifier <4> is not supported" error with GridIn in 2D.
    // We generate a 10x10 grid (h=0.1) and convert it to triangles.
    
    Triangulation<dim> mesh_quad;
    GridGenerator::subdivided_hyper_cube(mesh_quad, 10, 0.0, 1.0);
    GridGenerator::convert_hypercube_to_simplex_mesh(mesh_quad, mesh_serial);

    // Set boundary IDs manually:
    // Gamma_0 (Left, x=0) -> 0
    // Gamma_1 (Right, x=1) -> 1
    // Gamma_2 (Bottom, y=0) -> 2
    // Gamma_3 (Top, y=1) -> 3
    for (const auto &cell : mesh_serial.active_cell_iterators())
    {
      for (unsigned int f = 0; f < cell->n_faces(); ++f)
      {
        if (cell->face(f)->at_boundary())
        {
          ...
          if (std::abs(center[0] - 0.0) < 1e-9)
            cell->face(f)->set_boundary_id(0);
          else if (std::abs(center[0] - 1.0) < 1e-9)
            cell->face(f)->set_boundary_id(1);
          else if (std::abs(center[1] - 0.0) < 1e-9)
            cell->face(f)->set_boundary_id(2);
          else if (std::abs(center[1] - 1.0) < 1e-9)
...
Stokes::assemble()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      // [MODIFICA] Neumann BC is homogeneous (0) on Gamma_1, so no boundary integral needed.
      /*
      if (cell->at_boundary())
        {
          for (unsigned int f = 0; f < cell->n_faces(); ++f)
            {
              if (cell->face(f)->at_boundary() &&
                  cell->face(f)->boundary_id() == 1) // Gamma_1 is ID 1
                {
                   // ... (omitted)
                }
            }
  ...
  // Dirichlet boundary conditions.
  {
    ...
    // [MODIFICA] Dirichlet BCs:
    // Gamma_0 (ID 0): Inlet velocity g
    // Gamma_2 (ID 2) and Gamma_3 (ID 3): Zero velocity
    // Gamma_1 (ID 1): Neumann (handled implicitly)
    ...
    VectorTools::interpolate_boundary_values(dof_handler,
                                             ...
                                             // [Modifica] Sul boroso ID=0 i valori di dirichlet sono quelli di inlet_velocity

    boundary_functions.clear(); // Pulisce mappa precedente
    Functions::ZeroFunction<dim> zero_function(dim + 1);
    boundary_functions[2] = &zero_function; // Imposta velocità zero su Gamma_2
    boundary_functions[3] = &zero_function; // Imposta velocità zero su Gamma_3
    VectorTools::interpolate_boundary_values(dof_handler,
                                             boundary_functions,
                                             boundary_values,
                                             mask_velocity);
