...
Poisson1D::assemble()
{
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  // [MODIFICA] Aggiunto termine di trasporto u' (= shape_grad[0] * shape_value)
  ...
  // functions to
  {
    ...
    // [MODIFICA] Funzioni per le condizioni al contorno: u(0)=0 (id 0) e u(1)=1 (id 1)
...
Poisson1D::solve()
{
  ...
  // Since the system matrix is not symmetric (due to advection), we solve the
  // system using GMRES.
  // [MODIFICA] Uso di GMRES invece di CG per matrice non simmetrica
